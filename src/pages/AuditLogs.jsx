...
const AuditLogs = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      setLoading(true);
      // Kinukuha natin ang logs mula sa inyong database table, naka-sort mula pinakabago (desc)
      const { data, error } = await supabase
        .from('Audit_Logs') // Palitan ang pangalan ng table kung iba ang tawag niyo sa Supabase niyo
        .select('*')
      ...
      if (!error && data) {
        ...
        // Mock data para hindi blangko ang UI mo kung sakaling wala pang laman ang table niyo sa database
        setLogs([
